...
import brandData from '../data/brandData.json'; // Import brand data from JSON
...
// Define a type for newestPreOrderProducts data that includes products
type NewestPreOrderProducts = {
  products: {
    edges: Array<{node: ProductNode}>;
  };
};

type PreOrderDisplayProps = {
  poIsLoading: Signal<boolean>;
  newestPreOrderProducts: NewestPreOrderProducts;
  poBrandHandle: Signal<string | null>; // Defined as a signal
  prefetchPreOrderBrandProducts: (brandHandle: string) => void; // Corrected to a function type
};

// Create a React functional component named 'PreOrderDisplay' that takes 'newestPreOrderProducts' as a prop - newestPreOrderProducts is what is passed from ._index.tsx in the Homepage function
const PreOrderDisplay: React.FC<PreOrderDisplayProps> = ({
  poIsLoading,
  newestPreOrderProducts,
  poBrandHandle,
  prefetchPreOrderBrandProducts,
}) => {
  if (!newestPreOrderProducts) return null;

  // Transition effect based on poIsLoading signal
  const transitionClass = poIsLoading.valueOf() ? 'opacity-10' : 'opacity-100';
  const displayedPoProductsCount = useSignal(0); // Default to 0 to be written to by useEffect immediately after mount - count of products to be displayed
  ...
  // ***************** Scroll Settings for Brand Bar ********
  // Adjust scroll distance for small screens
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 768) {
        scrollDistance.value = 344;
      } else if (window.innerWidth > 1279) {
        scrollDistance.value = 653;
  ...
  // ********* Update Effect for how many product cards to render *****
  useEffect(() => {
    const updateDisplayedPoProductsCount = () => {
      const width = window.innerWidth;
      if (width >= 1280) {
        displayedPoProductsCount.value = 10; // xl
      } else if (width >= 1024) {
        displayedPoProductsCount.value = 8; // lg
      } else if (width >= 560) {
        displayedPoProductsCount.value = 6; // md & sm
      } else {
        displayedPoProductsCount.value = 4; // xs
      }
    };

    // Update the count immediately once the component mounts
    updateDisplayedPoProductsCount();

    // Updated the count on resize
    window.addEventListener('resize', updateDisplayedPoProductsCount);
    updateDisplayedPoProductsCount(); // Initial check

    return () =>
      window.removeEventListener('resize', updateDisplayedPoProductsCount);
  }, []);

  // Handler to update the selected brand ID
  const handleBrandClick = (newBrandHandle: string) => {
    poBrandHandle.value = newBrandHandle; // Directly update the signal's value
  };

  // Step 1: Filter for products that have the 'pre-order' tag.
  const preOrderTaggedProducts = newestPreOrderProducts.products.edges.filter(
    ({node: product}) => product.tags.includes('Pre-Order'),
  );

  // Step 2: From the filtered pre-order products, separate available and unavailable products.
  const availablePreOrderProducts = preOrderTaggedProducts.filter(
    ({node: product}) =>
      product.variants.nodes.some((variant) => variant.availableForSale),
  );

  const unavailablePreOrderProducts = preOrderTaggedProducts.filter(
    ({node: product}) =>
      !product.variants.nodes.some((variant) => variant.availableForSale),
  );

  // Step 3: Determine the products to display based on the displayedPoProductsCount.
  let displayProducts = availablePreOrderProducts.slice(
    0,
    displayedPoProductsCount.value,
  );
  if (displayProducts.length < displayedPoProductsCount.value) {
    displayProducts = [
      ...displayProducts,
      ...unavailablePreOrderProducts.slice(
        0,
        displayedPoProductsCount.value - displayProducts.length,
      ),
    ];
  }

  return (
    <div className=" max-w-[45rem] md:max-w-[53rem] lg:max-w-[70rem] xl:max-w-[85rem] min-w-[20rem] sm:min-w-[35rem] mx-auto my-2 p-2 sm:p-4 bg-slate-50 bg-opacity-90 rounded-3xl">
      <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold pb-4 text-center">
        Newest Pre-Orders!
      </h2>
      {/* Scroll Container w/ buttons */}
      <div className="flex flex-grow-0 mx-auto justify-center content-center items-center mb-4">
        {/* Left Arrow SVG */}
        <button
        ...
        <div
          ref={containerRef}
          className="flex overflow-x-auto whitespace-no-wrap min-w-60 max-w-[27.2rem] sm:w-[29.7rem] sm:min-w-[29.7rem] md:w-[39.5rem] md:min-w[39.5rem] lg:min-w-[46rem] lg:max-w-[46rem] lg:w-[46rem] xl:w-[62.5rem] xl:min-w-[62.5rem] relative border-4 sm:px-1 pb-1 pt-2 lg:px-2 lg:pb-2 lg:pt-3 border-slate-300 justify-between bg-blue-950 rounded-3xl transition-all duration-300 ease-in-out"
          ...
          {brandData.brands.map((brand) => (
            <img
              ...
              onMouseEnter={() => {
                ...
                prefetchPreOrderBrandProducts(brand.handle);
              }}
              className={`h-[4.58rem] sm:h-24 lg:h-28 xl:h-32 gap-1 md:gap-4 sm:mr-1 md:mr-4 cursor-pointer hover:scale-105 transition-all duration-300 drop-shadow-lg ease-in-out ${
                poBrandHandle.value === brand.handle
